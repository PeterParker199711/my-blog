...
const setup = () => {
  ...
  const fragmentShader = `
      ...
      // 👉 关键修复 1：把 tanh 函数写在最外面（main 函数之前）
      float tanh(float x) {
        float e = exp(2.0 * x);
        return (e - 1.0) / (e + 1.0);
      }
...
<template>
  <div v-if="!webGLSupported" class="pillar-container fallback" :class="className" :style="{ mixBlendMode }">
    WebGL not supported
  </div>
  <div v-else ref="containerRef" class="pillar-container" :class="className" :style="{ mixBlendMode }" />
</template>
